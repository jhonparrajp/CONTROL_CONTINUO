function [salida,ValoresK,pantalla,cortes]=criterioRouth(entrada)
...
salida=[coef simplify(Tabla(1:n,:))];
syms k
ValoresK= simplify([solve(salida(2*n)==0,k)]);
for i=1:n-1
ValoresK=[ValoresK simplify([solve(salida(2*n-i)==0,k)])];
%ValoresK=simplify([solve(salida(2*n)==0,k),solve(salida(2*n-1)==0,k)])
end
vpa(ValoresK,5)